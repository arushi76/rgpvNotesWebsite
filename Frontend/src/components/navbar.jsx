  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        {/* Desktop Links */}
        <ul className="hidden md:flex gap-8 text-cyan-100 font-semibold text-md">
          <li>
        ...
        <div className="md:hidden">
          <button
            ...
            className="flex flex-col w-8 h-8 justify-between"
          >
            <span className="block h-1 w-full bg-cyan-100 rounded"></span>
            <span className="block h-1 w-full bg-cyan-100 rounded"></span>
            <span className="block h-1 w-full bg-cyan-100 rounded"></span>
          </button>
        </div>
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="md:hidden bg-[#0c1a3b] border-t text-cyan-200 flex flex-col px-6 py-4 space-y-4">
          <Link
            to="/"
            onClick={() => setIsOpen(false)}
            className="hover:text-cyan-400 hover:underline underline-offset-4 transition duration-200"
          >
            Home
          </Link>
          <Link
